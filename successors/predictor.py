...
# features = dict()
# aa = list()
# aalist = ["A", "L", "R", "K", "N", "M", "D", "F", "C", "P", "Q", "S", "E", "T", "G", "W", "H", "Y", "I", "V"]
#
# with open(os.path.join('outputs/', 'aa_index_confidence', 'aa_indice_names.pkl'), "rb") as f:
...
#     sequences = dict()
#     length = 0
#     dt = open(dir + "/msa.fasta")  # for this dataset TODO
#     name = ""
#     act = ""
#     for line in dt.readlines():
#         line = line.strip()
...
#     sequences[name] = act
#
#     tree = Phylo.read(dir + "/ancestralTree.tree", "newick")
#
#     # name = (dir.split("/")[-1]).split("_")[1]
#     # name_query = (dir.split("/")[-1]).split("_")[0]  # TODO for P0A8U6
#     name = 'query'  # TODO special name for LGK setup
#     nodes = list()
#     for clade in tree.get_terminals():
#         path = tree.get_path(clade)
#         if name.lower() in str(path[-1]).lower():
#             for cl in path:
#                 nodes.append(">ancestral_" + str(cl.confidence))
#             nodes[-1] = ">" + path[-1].name
#             target_vec = sequences[nodes[-1]]
#             if EXTANT_PREDICTION:
#                 nodes = nodes[:-1]  # hide our target TODO comment in P0S8U6
#             break
#
#     vectors = list()
#     for i in range(length):
...
#         ft_list.append(dict())
#
#     scores = {"r2": {f: [None for _ in range(len(vectors))] for f in features if feature in selected_indices},
#               "slope": {f: [None for _ in range(len(vectors))] for f in features if feature in selected_indices},
#               "intercept": {f: [None for _ in range(len(vectors))] for f in features if feature in selected_indices},
#               "values": {f: [None for _ in range(len(vectors))] for f in features if feature in selected_indices},
#               "aminos": {f: [None for _ in range(len(vectors))] for f in features if feature in selected_indices},
#               "pred_vals": {f: [None for _ in range(len(vectors))] for f in features if feature in selected_indices},
#               "target_vec": {f: [None for _ in range(len(vectors))] for f in features if feature in selected_indices},
#               "sequentiality": {f: [None for _ in range(len(vectors))] for f in features if
#                                 feature in selected_indices},
#               "fluctuation": {f: [None for _ in range(len(vectors))] for f in features if feature in selected_indices},
#               "sequentiality2": {f: [None for _ in range(len(vectors))] for f in features if
#                                  feature in selected_indices}}
#     positions_to_keep, positions_detected = [], False
#     aa_on_the_last = []
#     for feature in features:
...
#                     act.append(float(features[feature][vc]))
#                     transitions_list.append(vc)
#                     lineage_hold.append(float(features[feature][vc]))
#             # keep tract of AA in the last ancestor
#             aa_on_the_last.append(vectors[i][-1])
#             # check the youngest ancestor, keep gap
#             if lineage_hold[-1] == "-":
#                 ft_list[i][feature] = "-"
#                 continue
#
...
#                     transitions_cnt += 1
#
#             if transitions_cnt < 3 and TRANSITION_RULE_ON:
#                 ft_list[i][feature] = lineage_hold[-1]
#                 continue
#
#             # modification weighting for regression nad 3+ transitions needed
#             if len(act) * 2 >= len(vectors[i]):
#                 # print(act, len(vectors[i]))
#                 x = list()
#                 y_w = list()
#                 vals_inds = list()
#                 for j in range(len(act)):
...
#
#     # make prediction base on individual AA indices
#     indices_pred = {k: ["", "", ""] for k in features if k != "gap" and k in selected_indices}  # 1st 2nd 3rd probable
#
#     for i in range(len(vectors)):
#         best_aa = list()
#         best_w = None
...
#         best_w_indice = None
#         for feature in ft_list[i]:
#             if feature not in selected_indices: continue
#             # if feature == "gap": continue
#
#             best_aa_indice = list()
#             keep_vals_aa = list()
#
...
#
#                 keep_vals_aa.append((aa, aa_dev))
#                 if best_w_indice is None or best_w_indice > aa_dev:
#                     best_w_indice = aa_dev
#                     best_aa_indice.append(aa)
#             keep_vals_aa.sort(key=lambda xx: xx[1])
#             best_aa_indice.reverse()
#             if scores['pred_vals'][feature][i] is not None:
#                 scores['pred_vals'][feature][i].append(keep_vals_aa[0][0])
#             indices_pred[feature][0] += keep_vals_aa[0][0]
#             indices_pred[feature][1] += keep_vals_aa[1][0]
#             indices_pred[feature][2] += keep_vals_aa[2][0]
#             # if predicted value has more residues including original one keep that original
#             the_best_aa, lowest_dev = [], keep_vals_aa[0][1]
#             for b in keep_vals_aa:
#                 if b[1] <= lowest_dev:
#                     the_best_aa.append(b[0])
...
#                     scores['pred_vals'][feature][i][-1] = aa_on_the_last[i]  # replace that last value
#
#     # create file for individual candidates 3 best candidates
#     for pred_i in range(3):
#         ot = open(dir + "/out/prediction_indices_{}.fasta".format(pred_i), "w")
#         for k, seq in indices_pred.items():
#             ot.write(">{}\n".format(k))
#             ot.write(seq[pred_i])
#             ot.write("\n")
...
#     for m in scores:
#         for feature, vals in scores[m].items():
#             with open(dir + "/out/{}_{}.pkl".format(m, feature), "wb") as f:
#                 pickle.dump(vals, f)
